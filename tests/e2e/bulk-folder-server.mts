...
import { shopProfiles } from './bulk-folder-fixtures.js';
...
const schema = 'e2e_bulk_folder_' + randomUUID().replaceAll('-', '');
...
const allowedRoot = resolve('.local/acceptance-20260914/bulk-folders');
const evidenceRoot = resolve(process.env.BULK_FOLDER_EVIDENCE_ROOT ?? '');
...
async function snapshot() {
  ...
  for (const table of ['products', 'work_orders', 'jobs', 'connections'])
    counts[table] = Number(
  ...
  return {
    ...
    batches,
    counts,
    syntheticShopProfiles: shopProfiles,
  };
}
async function close() {
  ...
  await pool.end();
  if (!/^e2e_bulk_folder_[a-f0-9]+$/.test(schema)) throw new Error('Unsafe schema cleanup');
...
try {
  ...
  for (const shop of shopProfiles)
    await pool.query(
      "INSERT INTO connections(id,environment,partner_id,shop_id,name,state) VALUES($1,'sandbox','910001',$2,$3,'connected')",
      [randomUUID(), shop.shopId, shop.name],
    );
